...
(function($) {
	...
	var Topbar = this.Topbar = Backbone.View.extend({
		...
		initialize: function() {
			this.$el.html('<img class="logo" src="/fx/amethyst.png" alt="Pokemon Amethyst" /><div class="maintabbarbottom"></div><div class="tabbar maintabbar"><div class="inner"></div></div><div class="userbar"></div>');
	...
	var RulesPopup = this.RulesPopup = Popup.extend({
		...
		initialize: function(data) {
			...
			buf += '<h2><font color=red>Lobby Chat Rules</font></h2>';
			buf += '<ol><font color=#2ECCFA><li>No spamming. This includes, but is not limited to, trolling/flaming, flooding, advertising, etc. We will not tolerate any of this. Please note the difference between the above and playfully joking (for example, lightly trolling a friend). We CAN tell the difference.</li><br><li>No inappropriate content. This includes keeping what you type appropriate as well. Keep in mind that COPPA has been lifted so these sites will be filled with minors, hence all pornography, vandalism, shock sites, discrimination, and cyber-bullying will be strictly prohibited. All violations will result in a swift mute or ban.</li><br><li>No excessive profanity. This will be primarily up to the enforcer, but generally, do not use profanity to display undesirable behaviors, such as aggression. If you wish to be safe, simply do not use any profanity.</li><br><li>No abusing privileges. Everyone is initially given the power to communicate freely. However, any abuse of those privileges will result in an immediate rescindment of those privileges. This includes abusing formatting. Use formatting sparingly.</li><br><li>5. Please be respectful of everyone on the server. We try to promote a positive environment here on Amethyst, so any impediments will be dealt with accordingly. This includes being respectful of your adversaries in battles. If you think a game went poorly either due to the strategy your opponent employed or luck, you do not need to say "good game," but you should not curse him/her.</li><br><li>Please be respectful of figures of authority especially. They have attained their status due to their consistently positive presence on the server, and as such, demand a certain respect. If an authority figure warns you for unacceptable behavior, please listen respectfully to avoid further complications. As a side note, please be respectful of staff members time as they are often busy and do not always have time to battle and/or respond to a pm.</li><br><li>Please note that no one is exempt from the rules. The rules apply to everyone, staff members included. In fact, as authority figures, members of staff should tread even more carefully so as to establish a strong standard for the rest of the server to follow.</li></ol></font>';
			if (warning) {
